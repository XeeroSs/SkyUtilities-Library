package com.xeross.skyutilities.helpers.schematics.blocks;
...
public class ArenaBlock {
                            ...
                            while (iter.hasNext()) {
                                ...
                                if (format.startsWith("{")) {
                                    ...
                                    while (formatIter.hasNext()) {
                                        Entry entry = (Entry) formatIter.next();
    ...
    public static void setBlockInNativeWorld(Block block, int blockId, byte data, boolean applyPhysics) {
    ...
    public void pasteDirt(NMSAbstraction nms, Location blockLoc, boolean usePhysics) {
        Block block = new Location(blockLoc.getWorld(), x, y, z).add(blockLoc).getBlock();
        // setBlockInNativeWorld(block, 1, 0, usePhysics);
    }
    ...
    //@SuppressWarnings("deprecation")
    @SuppressWarnings("deprecation")
    public void paste(NMSAbstraction nms, Location blockLoc, boolean usePhysics, boolean isBridge) {
        // Only paste air if it is below the sea level and in the overworld
        Block block = new Location(blockLoc.getWorld(), x, y, z).add(blockLoc).getBlock();
        setBlockInNativeWorld(block, typeId, data, usePhysics);
        //  block.getState().update();
        //     nms.setBlockSuperFast(block, typeId, data, usePhysics);
        // block.setTypeIdAndData(typeId, data, usePhysics);
...
/*        if (material != Material.GRASS
                ...
                && material != Material.WOOD && material != Material.CLAY) {
            System.out.println("Type : " + material.name() + "(" + material.getId() + ") [Location: X = " + block.getX() + ", Y = " + block.getY() +
                    ", Z = " + block.getZ() + "]");
        }*/
        if (signText != null) {
