...
public class BannerBlock {
    ...
    private static final HashMap<String, PatternType> patternKey;
    ...
    public void prep(Map<String, Tag> tileData) {
        ...
        try {
            ...
            // int test = ((IntTag) tileData.get("Base")).getValue();
            //      System.out.println(((IntTag) tileData.get("Base")).getValue());
            //    System.out.println(baseColor);
            //  System.out.println("TEST = "+DyeColor.getByDyeData((byte) test));
            // baseColor);
            // baseColor green = 10
            bannerBaseColor = DyeColor.getByDyeData((byte) baseColor);
            // System.out.println(bannerBaseColor);
            ...
            if (patterns != null) {
                for (Tag pattern : patterns.getValue()) {
                    ...
                    if (pattern instanceof CompoundTag) {
                        ...
                        int markColor = /*15 -*/ ((IntTag) patternValue.get("Color")).getValue();
                        // System.out.println(((IntTag) patternValue.get("Color")).getValue() + " - 15 = " + (15 - ((IntTag) patternValue.get("Color")).getValue()));
                        // mark.getValue());
                        // markColor);
                        DyeColor dColor = DyeColor.getByDyeData((byte) markColor);
